...
function player1move(gameid, pl1, pl2, gms) {
  console.log("from pl1move", gameid, pl1, pl2, gms)
  if (whosechance > 10) {
    return;
  }
...
function startgame(gameid: number): void{
  const game: any[] = gamestate.filter((obj => obj.id == gameid));
  const pl1: string = game[0].gamedata.Player1;
  const pl2: string = game[0].gamedata.Player2;
  const gms: string[] = game[0].gamedata.Gamestate
      ...
      if(data == 1){
        ...
        socket.on("gameid-choice", (data: number) => {
          ...
          socket.on("player1-move-index", (indexlist: any[]) => {
            console.log("hello from pl1");
            if(whosechance < 10){
              gamestate[indexlist[0]].gamedata.Gamestate[indexlist[3]] = "X";
          let game: any[] = gamestate.filter((obj => obj.id == indexlist[0]))
          let gms: string[]= game[0].gamedata.Gamestate;
          whosechance = whosechance + 1;
          console.log("from pl1 update", gms);
          player2move(indexlist[0], indexlist[1], indexlist[2], gms);
          }
        });
        socket.on("player2-move-index", (indexlist: any[]) => {
          if(whosechance < 10){          gamestate[indexlist[0]].gamedata.Gamestate[indexlist[3]] = "O";
          let game: any[] = gamestate.filter((obj => obj.id == indexlist[0]))
          let gms: string[] = game[0].gamedata.Gamestate;
          whosechance = whosechance + 1;
          console.log("from pl2 update", gms);
